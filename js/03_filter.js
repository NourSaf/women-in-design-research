...
for(let i = 0; i < timeUniqueList.length; i++){
  const button = document.createElement("button");
  button.innerText = timeUniqueList[i];
  button.classList.add("filterbtnI")
  ...
  button.addEventListener("click",(e) => {
    ...
    designerList = designers.filter((newE) => newE.time === e.target.innerText)
    itemBoxfuction()
  })
